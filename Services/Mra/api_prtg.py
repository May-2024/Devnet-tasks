...
def get_status_prtg(db_devnet_data):
    ...
    try:
        for device in db_devnet_data:
            logging.info(f"{device['device']} {device['sensor']}")
            id_device = device["id_device"]
            api_endpoint = os.getenv("DATA_INTERFACE").format(id_device=id_device, username=PRTG_USERNAME, password=PRTG_PASSWORD)
            api_endpoint_ping = os.getenv("DATA_PING").format(id_device=id_device, username=PRTG_USERNAME, password=PRTG_PASSWORD)
